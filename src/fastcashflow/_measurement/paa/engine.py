...
from fastcashflow._measurement.paa.results import (
    Measurement, Aggregate, SettlementMovement, SettlementAggregate,
...
def measure_stream(
    ...
    *,
    ...
    return _stream_policies_coverages(
        ...
        measure_fn=lambda mp: measure(mp, basis, revenue_basis=revenue_basis,
                                          full=False),
    )
...
def measure_aggregate(
    ...
    *,
    ...
    for start in range(0, n_mp, chunk_size):
        ...
        m = measure(model_points.subset(idx), basis,
...
def measure_inforce(
    ...
    *,
    ...
    (:func:`measure`) and the LRC trajectory is sliced at ``elapsed_months``
    ...
    m = measure(model_points, basis, revenue_basis=revenue_basis, full=True)
    n_mp = m.lrc.shape[0]
