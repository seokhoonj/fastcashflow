...
def _project_kernel(rates_by_year, term_months, lapse, monthly_premium,
    ...
    Mortality is supplied per policy year (``rates_by_year``); the attained
    age, and so the rate, changes only once every twelve months.
    """
...
def project_cashflows(mps: ModelPointSet, asmp: Assumptions) -> CashflowProjection:
    ...
    evaluates the mortality assumption. Mortality is evaluated on the
    ``(n_mp, n_years)`` distinct-age grid, not the full ``(n_mp, n_time)``
    grid -- the attained age changes only once a year, so this is an
    identical result for a twelfth of the (transcendental) work.
    """
    n_time = int(mps.term_months.max())     # months 0 .. n_time-1
    n_years = (n_time + 11) // 12
    months = np.arange(n_time)

    age_by_year = mps.issue_age[:, None] + np.arange(n_years)[None, :]
    rates_by_year = np.ascontiguousarray(
        asmp.mortality_monthly(age_by_year), dtype=np.float64
    )
