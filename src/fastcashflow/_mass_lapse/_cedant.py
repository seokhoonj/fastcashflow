"""Mass-lapse reinsurance -- cedant side (standard-formula capital relief).

The deterministic standard-formula view: the mass-lapse own-funds strain
(:func:`lapse_loss_density`), the lapse excess-of-loss treaty (:class:`LapseXL`),
the windowed claim, the standard-formula capital relief, the counterparty-
default charge on the recoverable, and the full cedant solvency relief. The
reinsurer-side pricing (:mod:`._reinsurer`) and the report (:mod:`._report`)
build on these; this module imports neither, so it is the package's base layer.
"""
