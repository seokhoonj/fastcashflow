...
def _bases():
    ...
    return {
        "DEATH_A":   _basis(state_machine=_M_DEATH),
        "DIAG_A":    _basis(state_machine=_M_DEATH,
                            coverages=(fcf.CoverageRate("CANCER_DIAGNOSIS", _cancer),)),
        "MORB_A":    _basis(state_machine=_M_DEATH,
                            coverages=(fcf.CoverageRate("CANCER_INPATIENT", _cancer),)),
        "ANN_A":     _basis(state_machine=_M_DEATH),
        "WAIVER_A":  _basis(state_machine=Model.from_preset("ACTIVE_WAIVER"),
                            waiver_incidence_annual=_inc),
        "REINCID_A": _basis(state_machine=_M_REINCID,
                            ci_incidence_annual=_ci1, ci_reincidence_annual=_ci2),
        "DI_A":      _basis(state_machine=_M_DI,
                            waiver_incidence_annual=_inc, disability_recovery_annual=_di_rec),
        "LTC_A":     _basis(state_machine=_M_LTC,
                            waiver_incidence_annual=_inc,
                            state_mortality_annual={"dth_care": _care_mort}),
        "SURR_A":    _basis(state_machine=_M_DEATH,
                            surrender_value_curve=np.full(240, 0.8),
                            surrender_value_basis="cum_premium_factor"),
        "RENEW_A":   _basis(state_machine=_M_DEATH,
                            coverages=(fcf.CoverageRate("CANCER_INPATIENT", _cancer),)),
    }
