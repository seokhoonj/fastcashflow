...
_unit = ModelPoints(
    ...
    benefits={"DEATH": np.array([10_000.0])}, count=np.ones(1),
...
def book(*, benefits, em_close, counts, prior_counts, prior_csms, prior_lcs,
    ...
    mp = ModelPoints(
        ...
        benefits={"DEATH": rep(benefits)}, count=rep(counts),
...
def test_rejects_missing_issue_date_for_default_cohort():
    ...
    mp = ModelPoints(
        ...
        benefits={"DEATH": np.asarray(BENEFITS)}, count=np.array([0.5, 0.5]),
...
def test_two_goc_rows_each_equals_its_standalone_settle():
    ...
    mp = ModelPoints(
        ...
        term_months=np.full(n, 120, dtype=np.int64),
        premium_term_months=np.full(n, 120, dtype=np.int64),
        benefits={"DEATH": np.array([10_000.0, 50_000.0])},
...
def _two_product_book(n_each=1):
    ...
    mp = ModelPoints(
        ...
        term_months=np.full(n, 120, dtype=np.int64),
        premium_term_months=np.full(n, 120, dtype=np.int64),
        benefits={"DEATH": np.full(n, 50_000.0)}, count=np.full(n, 0.5),
