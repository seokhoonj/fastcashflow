...
def test_alpha_fixed_row_lands_in_alpha_fixed_primitive():
    ...
    rows = (ExpenseRow("acquisition", "alpha_fixed", 50_000.0),)
    (alpha_pro_rata, alpha_fixed, beta_pro_rata,
...
def test_alpha_pro_rata_row_lands_in_alpha_pro_rata_primitive():
    ...
    rows = (ExpenseRow("acquisition", "alpha_pro_rata", 1.20),)
    (alpha_pro_rata, alpha_fixed, beta_pro_rata,
     gamma_fixed, lae_pro_rata) = derive_expense_components(rows, 12)
...
def test_beta_pro_rata_row_lands_in_beta_pro_rata_primitive():
    ...
    rows = (ExpenseRow("collection", "beta_pro_rata", 0.01),)
    (alpha_pro_rata, alpha_fixed, beta_pro_rata,
     gamma_fixed, lae_pro_rata) = derive_expense_components(rows, 12)
...
def test_gamma_fixed_grows_with_inflation():
    ...
    """
    rows = (
        ExpenseRow("maintenance", "gamma_fixed", 36_000.0),
    )
    n_time = 24
...
def test_lae_pro_rata_grows_with_inflation():
    ...
    rows = (ExpenseRow("LAE", "lae_pro_rata", 0.02),)
    n_time = 24
...
def test_multiple_rows_sum_into_each_primitive():
    ...
    rows = (
        ExpenseRow("acquisition", "alpha_fixed", 50_000.0),
        ExpenseRow("acquisition_extra", "alpha_fixed", 10_000.0),
        ExpenseRow("maintenance", "gamma_fixed", 36_000.0),
        ExpenseRow("overhead", "gamma_fixed", 12_000.0),
    )
...
def _basis_rows():
    ...
    return fcf.Assumptions(
        ...
        expense_rows=(
            ExpenseRow("acquisition",  "alpha_fixed",    120_000.0),
            ExpenseRow("acquisition",  "alpha_pro_rata",        0.20),
            ExpenseRow("collection",   "beta_pro_rata",         0.02),
            ExpenseRow("maintenance",  "gamma_fixed",       36_000.0),
        ),
    )
...
def test_lae_pro_rata_row_lifts_expense():
    ...
    with_lae = dataclasses.replace(
        ...
        expense_rows=base_rows.expense_rows + (
            ExpenseRow("LAE", "lae_pro_rata", 0.02),
        ),
    )
