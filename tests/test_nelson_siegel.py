...
def test_evaluate_hand_calc():
    ...
    """
    y = fcf.curves.nelson_siegel_svensson(2.0, **_PARAMS)
...
def test_short_and_long_limits():
    ...
    y0 = float(fcf.curves.nelson_siegel_svensson(1e-9, **_PARAMS))
    ylong = float(fcf.curves.nelson_siegel_svensson(1e6, **_PARAMS))
...
def test_scalar_in_scalar_out_and_vector():
    assert np.ndim(fcf.curves.nelson_siegel_svensson(5.0, **_PARAMS)) == 0
    v = fcf.curves.nelson_siegel_svensson(np.array([1.0, 5.0, 10.0]), **_PARAMS)
    assert v.shape == (3,)


def test_nelson_siegel_is_svensson_with_zero_second_hump():
    taus = np.array([0.5, 2.0, 10.0, 30.0])
    ns = fcf.curves.nelson_siegel(taus, beta0=0.04, beta1=-0.02, beta2=0.01, lambda1=2.0)
    nss = fcf.curves.nelson_siegel_svensson(taus, beta0=0.04, beta1=-0.02, beta2=0.01,
...
def test_negative_lambda_rejected():
    with pytest.raises(ValueError, match="positive"):
        fcf.curves.nelson_siegel_svensson(5.0, beta0=0.04, beta1=0.0, beta2=0.0,
...
def test_fit_recovers_a_synthetic_curve():
    ...
    y = fcf.curves.nelson_siegel_svensson(taus, **_PARAMS)
    ns = fcf.curves.fit_nelson_siegel_svensson(taus, y)
    fitted = ns(taus)
...
def test_fit_smooths_through_noise():
    ...
    clean = fcf.curves.nelson_siegel_svensson(taus, **_PARAMS)
    noisy = clean + rng.normal(0.0, 2e-4, taus.shape)
    ns = fcf.curves.fit_nelson_siegel_svensson(taus, noisy)
    resid = ns(taus) - noisy
...
def test_evaluate_rejects_negative_maturity():
    with pytest.raises(ValueError, match="non-negative"):
        fcf.curves.nelson_siegel_svensson(np.array([1.0, -2.0]), **_PARAMS)
...
def test_fit_rejects_degenerate_grid_or_zoom():
    ...
    with pytest.raises(ValueError, match="grid"):
        fcf.curves.fit_nelson_siegel_svensson(taus, y, grid=1)
    with pytest.raises(ValueError, match="zoom_rounds"):
        fcf.curves.fit_nelson_siegel_svensson(taus, y, zoom_rounds=0)
    with pytest.raises(ValueError, match="non-negative"):
        fcf.curves.fit_nelson_siegel_svensson(np.array([-1.0, 1, 2, 5]), y[:4])


def test_fit_requires_four_points_and_matching_shapes():
    with pytest.raises(ValueError, match="at least 4"):
        fcf.curves.fit_nelson_siegel_svensson(np.array([1.0, 5.0, 10.0]),
                                       np.array([0.03, 0.035, 0.04]))
    with pytest.raises(ValueError, match="same length"):
        fcf.curves.fit_nelson_siegel_svensson(np.array([1.0, 2, 3, 4]),
                                       np.array([0.03, 0.035, 0.04]))
