...
def test_cost_of_capital_ra_hand_calc():
    ...
    mp = ModelPoints.single(40, 60_000.0, 60, benefits={0: 1e8}, benefit_patterns=PATTERNS)
    coc_rate = 0.06
    ...
    full = cl.discount_start[1]                       # (1 + i)^-1
    capital = cl.ra[0]
    cap_pv0 = float(np.sum(capital * full ** np.arange(capital.shape[0])))
    assert np.isclose(coc.ra[0, 0], coc_rate * cap_pv0)
...
def test_coc_ra_scales_with_the_rate():
    ...
    assert np.isclose(coc2.ra[0, 0], 2.0 * coc1.ra[0, 0])
...
def test_coc_ra_differs_from_confidence_level():
    ...
    assert 0.1 < coc.ra[0, 0] / cl.ra[0, 0] < 5.0
