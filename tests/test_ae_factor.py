...
def test_no_ae_factor_sheet(tmp_path):
    ...
    assert asmp.mortality_annual(s, a, d)[0] == 0.001
    assert asmp.riders[0].rate(s, a, d)[0] == 0.02
...
def test_scalar_ae_factor_per_rider(tmp_path):
    ...
    assert np.isclose(asmp.riders[0].rate(s, a, d)[0], 0.03)
...
def test_ae_factor_on_main_mortality(tmp_path):
    ...
    asmp = _segment(p)
    s = np.array([0]); a = np.array([30]); d = np.array([0])
    assert np.isclose(asmp.mortality_annual(s, a, d)[0], 0.001 * 0.80)
...
def test_ae_factor_composes_with_age_shift(tmp_path):
    ...
    assert np.isclose(asmp.mortality_annual(s, a, d)[0], 0.001 * 16 * 0.5)
