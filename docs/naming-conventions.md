...
| `coverages` | rate-driven 담보 registry: `coverage_code → rate_table`. 모든 상품 공통 (product 별로 다른 calibration 필요시 `CANCER_HEALTH`, `CANCER_WHOLELIFE` 처럼 code 분리). `coverage_name` / `benefit_pattern` 은 별도 `benefit_patterns.csv` (담보 카탈로그) 에 |
...
| 파일 | 역할 |
|---|---|
| `benefit_patterns.csv` | 담보 카탈로그 (`coverage_code → benefit_pattern` 분류). 5종 fixed pattern (DEATH / MORBIDITY / DIAGNOSIS / ANNUITY / MATURITY). `assumptions.xlsx` 와 분리 — 연 1회 미만으로 바뀜 |
