...
사업비는 **2 축** -- `category` (무엇을 위한 비용) × `base` (무엇에 비례).

| 컬럼 | 의미 |
|---|---|
| `table_id` | 표 식별자 (예: `EXPENSE_TERM_FC`) |
| `category` | 비용 카테고리 -- `acquisition` / `maintenance` / `collection` / `lae` 중 하나. 타이밍·인플레 적용 여부를 결정 |
| `base` | 비례 기준 -- `per_policy` / `premium` / `surrender_value` / `face` / `claim` 중 하나 |
| `value` | 값 (`per_policy` 는 계약당 정액, 나머지는 비율 0..1) |

앞의 세 `category` 는 한국 actuarial α / β / γ 분류에 대응 -- acquisition = α
신계약비, maintenance = β 유지비, collection = γ 수금비; `lae` 는 LAE (Loss
Adjustment Expense, 손해사정비). 유효한 `(category, base)` 짝과 적용 시점:

| (category, base) | 의미 | 적용 시점 |
|---|---|---|
| (`acquisition`, `per_policy`) | 신계약비 계약당 정액 (`inforce[0] × value`) | t = 0 |
| (`acquisition`, `premium`) | 신계약비 보험료 비례 (`annualized_premium × value`) | t = 0 |
| (`maintenance`, `per_policy`) | 유지비 계약당 정액 (`inforce × value / 12`) | 매월 |
| (`maintenance`, `premium`) | 유지비 보험료 비례 (`premium × value`) | 매월 (납입 중) |
| (`maintenance`, `surrender_value`) | 유지비 보유 해약환급금 비례 (`inforce 해약환급금 × value / 12`). 한국 `%해약식준비금(연동)` 유지비. `surrender_value_curve` 필요, 계좌형(UL/VFA) 미적용(`fund_fee` 가 이미 부과) | 매월 |
| (`maintenance`, `face`) | 유지비 주계약 가입금액 비례 (`주계약 coverage_amount × value / 12`). 주계약을 `coverage_is_main` 으로 표시 필요 | 매월 |
| (`collection`, `premium`) | 수금비 보험료 비례 -- 형태는 maintenance-premium 과 동일, 카테고리만 수금 | 매월 (납입 중) |
| (`lae`, `claim`) | 손해사정비 청구 비례 (`(claim + morbidity) × value`) | 매월 |

(`account_value` base 는 향후 유지비 base 로 예약.)

내부 kernel-side primitive 이름은 `<category>_<base>` (예: `maintenance_per_policy`,
`lae`) -- 외부 입력과 동일 어휘. 글로벌 `expense_inflation` (segments 의
`inflation_table` 참조) 은 (`maintenance`, `per_policy`)·(`maintenance`, `face`)·
(`lae`, `claim`) 에만 적용 -- acquisition 은 t=0 일회성, `premium` base 는 이미
...
| `rate` | `discount`: 연 할인율 · `inflation`: 연 사업비 인플레이션 |
...
- `expense_table` — `expense_tables` 시트의 `table_id` 참조 (3.3 §).
  ...
  `category` / `base` / `value` 가 `(category, base)` 짝으로 kernel-side
  primitive 에 dispatch (acquisition/maintenance/collection/lae ×
  per_policy/premium/surrender_value/face/claim).
  비어 있으면 사업비 zero.
...
**Optional `state_machine` 컬럼** (문자열, 기본 None):
세그먼트가 어떤 상태기계 (Model) 를 쓰는지 enum 으로 선택.
`Model.from_preset` 사전의 키 (문자열) 를 값으로 넣음.

| 값 | 의미 |
|---|---|
| (빈 셀) | `Basis.state_machine = None`. 다중 상태 mechanic 이 트리거되면 (납입면제 / 모델포인트의 state 컬럼 비-0) 자동으로 `ACTIVE_WAIVER_MODEL` 사용 |
...
`Model` 개체를 직접 만들어 `Basis(state_machine=...)` 에 주입.

샘플 워크북의 `_DEFAULTS` 행은 `state_machine = ACTIVE_WAIVER` 로 설정되어 있어
