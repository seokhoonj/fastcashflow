...
| `surrender_value_pro_rata` | 유지비 -- 보유 해약환급금 비례 (`inforce 해약환급금 × value / 12`). 한국 `%해약식준비금(연동)` 유지비. `surrender_value_curve` 필요, 계좌형(UL/VFA) 에는 미적용(`fund_fee` 가 이미 부과) | 매월 |

글로벌 `expense_inflation` (segments 의 `inflation_table` 참조) 은
`gamma_fixed` 와 `lae_pro_rata` 에만 적용 -- 두 alpha basis 는 t=0
일회성, `beta_pro_rata` 는 이미 보험료 자체가 시간 변동, 그리고
`surrender_value_pro_rata` 는 해약환급금 곡선 자체가 성장하므로
모두 이중 적용 회피.
