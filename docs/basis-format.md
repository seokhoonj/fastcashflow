...
| `category` | 비용 카테고리 -- `acquisition` / `maintenance` / `collection` / `claims` 중 하나. 타이밍·인플레 적용 여부를 결정 |
| `base` | 비례 기준 -- `per_policy` / `premium` / `surrender_value` / `claim` 중 하나 |
| `value` | 값 (`per_policy` 는 계약당 정액, 나머지는 비율 0..1) |

`category` 는 한국 actuarial α / β / γ 분류에 대응 -- acquisition = α 신계약비,
maintenance = β 유지비, collection = γ 수금비, claims = LAE (Loss Adjustment
Expense, 손해사정비). 유효한 `(category, base)` 짝과 적용 시점:

| (category, base) | α/β/γ | 의미 | 적용 시점 |
|---|---|---|---|
| (`acquisition`, `per_policy`) | α' | 신계약비 계약당 정액 (`inforce[0] × value`) | t = 0 |
| (`acquisition`, `premium`) | α | 신계약비 보험료 비례 (`annualized_premium × value`) | t = 0 |
| (`maintenance`, `per_policy`) | β | 유지비 계약당 정액 (`inforce × value / 12`) | 매월 |
| (`maintenance`, `premium`) | β | 유지비 보험료 비례 (`premium × value`) | 매월 (납입 중) |
| (`maintenance`, `surrender_value`) | β | 유지비 보유 해약환급금 비례 (`inforce 해약환급금 × value / 12`). 한국 `%해약식준비금(연동)` 유지비. `surrender_value_curve` 필요, 계좌형(UL/VFA) 미적용(`fund_fee` 가 이미 부과) | 매월 |
| (`collection`, `premium`) | γ | 수금비 보험료 비례 -- 형태는 β-premium 과 동일, 카테고리만 수금 | 매월 (납입 중) |
| (`claims`, `claim`) | LAE | 손해사정비 청구 비례 (`(claim + morbidity) × value`) | 매월 |

(`face` / `account_value` base 는 향후 유지비 base 로 예약.)

글로벌 `expense_inflation` (segments 의 `inflation_table` 참조) 은
(`maintenance`, `per_policy`) 와 (`claims`, `claim`) 에만 적용 -- acquisition 은
t=0 일회성, `premium` base 는 이미 보험료 자체가 시간 변동, 그리고
`surrender_value` 는 해약환급금 곡선 자체가 성장하므로 모두 이중 적용 회피.
...
- `expense_table` — `expense_tables` 시트의 `table_id` 참조 (3.3 §).
  ...
  primitive 에 dispatch (acquisition/maintenance/collection/claims ×
  per_policy/premium/surrender_value/claim).
  비어 있으면 사업비 zero.
