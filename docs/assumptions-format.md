...
| `segments` | (상품 × 채널) 세그먼트별 어느 표를 쓸지 + 스칼라 파라미터 (`alpha_*` / `beta_*` / `gamma_flat` 사업비 포함) |
| `coverages` | 상품별 특약 부착 (coverage_code, benefit_pattern, rate_table) |
...
### 3.3 사업비 (`alpha_*` / `beta_*` / `gamma_flat`) — `segments` 시트

신사업비 / 보험료 비례 / 유지비는 **별도 시트 없이** `segments` 의 스칼라
컬럼으로 처리합니다 (4절 참조):

- `alpha_pct` / `alpha_flat` — 신사업비. 가입 시 일회성. 보험료 비례 (`pct`)
  + 정액 (`flat`).
- `beta_pct` — 보험료 비례 유지/관리비. 매월 보험료에 곱함.
- `gamma_flat` — 정액 유지비. 매월 in-force 에 곱함. 실질 단가 (인플레이션은
  `inflation_tables` 가 별도로 키움).

이 단순화는 단일 segment 안에서 사업비가 가입연도 / 경과기간 / 손해율 에
대해 평탄 (flat) 임을 가정합니다. 한국 시장의 풍부한 expense framework
(acquisition / maintenance / collection / claim handling / overhead × duration ×
channel) 을 다 잡으려면 별도 expense_tables 시트가 필요하지만, 그건 미래
확장으로 둡니다.
...
| 값 | 의미 |
|---|---|
| (빈 셀) | `Assumptions.state_model = None`. 다중 상태 mechanic 이 트리거되면 (납입면제 / 모델포인트의 state 컬럼 비-0) 자동으로 `WAIVER_MODEL` 사용 |
| `WAIVER` | 2-state Markov (ACTIVE / WAIVER). 가장 흔한 한국 protection 상품 형태. 현재 등록된 유일한 키 |

향후 라이브러리 버전업에서 추가 등록 가능 (`CANCER_REINCIDENCE`,
`DISABILITY`, `LTC_GRADES` 등). 등록되지 않은 키를 적으면 read_assumptions
...
main mortality는 `coverage_code = "DEATH_MAIN"` 으로 표기 (`coverages` 시트의
...
TERM_A    GA       DEATH_MAIN     0.8         # CI 80%
```
...
**전역 담보 정의** — 각 `coverage_code` 의 유형과 요율표. 모든 상품이 같은
담보 정의를 공유합니다 (HEALTH 의 `CANCER` 와 WHOLE_LIFE 의 `CANCER` 가
동일 incidence rate 사용).

| 컬럼 | 의미 |
|---|---|
| `coverage_code` | 담보 코드 (model_point 의 coverage row 와 매칭) |
| `coverage_name` | 담보명 (사람용 메모 — 엔진은 안 읽음) |
| **`benefit_pattern`** | **engine 의 계산 방식 routing key** — 6 개 고정 vocabulary |
| `rate_table` | 요율 기반 (`DEATH`·`MORBIDITY`·`DIAGNOSIS`) 이면 `incidence_rate_tables` 의 `table_id`; 나머지는 빈칸 |

### benefit_pattern 의 6 종 — engine 계산 방식 결정

**중요: `benefit_pattern` 값이 engine 의 cash flow 계산 패턴을 결정합니다.**
사용자는 각 담보가 어떤 `benefit_pattern` 인지 선언만 하고, engine 이 그 type 의
계산 방식 (몇 회 지급 / decrement / pool / inforce 곱 etc.) 을 hardcoded 로 적용.

| `benefit_pattern` | engine 계산 패턴 | 사용 case |
|---|---|---|
| `DEATH_MAIN` | in-force decrement × `death_benefit`. `mortality_table` (segments) 가 driver. `rate_table` 빈칸 | 주계약 사망 |
| `DEATH` | incidence (coverage 의 `rate_table` lookup) × benefit, **non-decrementing** | 사망 특약 (재해사망 ADB 등) |
...
값은 **반드시 위 6 개 중 하나**. 다른 값은 reader error. 새 패턴이 필요하면
engine 의 새 `TYPE_*` 추가 + projection kernel 의 routing branch 추가 작업
(library maintainer 의 영역).

**product 별 다른 정의가 필요한 경우** (예: HEALTH 의 cancer 와 WHOLE_LIFE 의
cancer 가 진짜 다른 base rate calibration) → 다른 `coverage_code` 로 분리
