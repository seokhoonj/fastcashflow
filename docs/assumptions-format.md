...
| `benefit_type` | `DEATH_MAIN` / `DEATH` / `MORBIDITY` / `DIAGNOSIS` / `ANNUITY` / `MATURITY` |
| `rate_table` | 요율 기반 (`DEATH`·`MORBIDITY`·`DIAGNOSIS`) 이면 `incidence_rate_tables` 의 `table_id`; 나머지는 빈칸 |

- `DEATH_MAIN` — 주계약 사망. 요율은 segments 의 `mortality_table` 이 구동하므로
  `rate_table` 은 빈칸.
- `ANNUITY` / `MATURITY` — 생존급부. 요율 없음 (`maturity_benefit`,
  `annuity_payment` model_point 스칼라로 구동).
