...
**키 컬럼**: `product_code`, `channel_code`.
...
```
product_code  channel_code  mortality_table  lapse_table  ...  expense_table  ra_confidence  mortality_cv
...
필수 컬럼: `product_code`, `channel_code`, `coverage_code`, `factor`.
...
**rate-driven 담보 registry** — 각 `coverage_code` 의 요율표. 모든 상품이
...
> **분리된 두 파일**: `coverage_code` 의 `benefit_pattern` (계산 routing) 과
> `coverage_name` (사람용 메모) 는 별도 **`benefit_patterns.csv`** (회사
> 카탈로그) 에 있습니다 — `assumptions.xlsx` 는 actuarial basis 만,
> 카탈로그는 연 1회 미만으로 바뀌는 안정적 분류. 자세한 건
> `docs/cookbook/basics/benefit-patterns-catalog.md` 참조.

### benefit_pattern 의 5 종 (`benefit_patterns.csv` 에서 분류)

**중요: `benefit_pattern` 값이 engine 의 cash flow 계산 패턴을 결정합니다.**
사용자는 각 담보가 어떤 `benefit_pattern` 인지 카탈로그에서 선언만 하고,
engine 이 그 패턴의 계산 방식 (몇 회 지급 / depleting pool / inforce 곱 etc.)
을 hardcoded 로 적용.
...
아니라 `benefit_patterns.csv` 에만 등록하고, model_points 의 `annuity_payment`
