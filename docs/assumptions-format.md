...
`inflation_table`, `surrender_value_table` (선택), `expense_table` (권장).
...
- `expense_table` — `expense_tables` 시트의 `table_id` 참조 (3.3 §).
  채워지면 item-form 사업비 ledger 가 우선 적용되고 아래 alpha/beta/gamma
  스칼라는 reader 가 0 으로 둡니다. 비어 있으면 스칼라 경로.
...
TERM_LIFE_A    GA                        LAPSE_GA     ...  150000
TERM_LIFE_A    FC                        LAPSE_FC     ...   80000
```

- `TERM_LIFE_A / GA` — `mortality_table` 등 빈 칸은 `defaults`에서 상속
  (`MORTALITY_STD`, `ra_confidence` 0.75, `mortality_cv` 0.10). `lapse_table`과
  `alpha_flat`만 행에서 지정.
- `TERM_LIFE_A / FC` — 같은 패턴, lapse와 acquisition만 다름.
...
담보 정의를 공유합니다 (HEALTH_A 의 `CANCER` 와 WHOLE_LIFE_A 의 `CANCER` 가
...
| **`benefit_pattern`** | **engine 의 계산 방식 routing key** — 5 개 고정 vocabulary |
| `rate_table` | 요율 기반 (`DEATH`·`MORBIDITY`·`DIAGNOSIS`) 이면 `incidence_rate_tables` 또는 `mortality_tables` 의 `table_id`; 나머지는 빈칸 |

### benefit_pattern 의 5 종 — engine 계산 방식 결정

**중요: `benefit_pattern` 값이 engine 의 cash flow 계산 패턴을 결정합니다.**
사용자는 각 담보가 어떤 `benefit_pattern` 인지 선언만 하고, engine 이 그 패턴의
계산 방식 (몇 회 지급 / depleting pool / inforce 곱 etc.) 을 hardcoded 로 적용.

| `benefit_pattern` | engine 계산 패턴 | 사용 case |
|---|---|---|
| `DEATH` | incidence (coverage 의 `rate_table` lookup) × benefit, **non-decrementing** | 일반 사망 / 상해사망 / 재해사망 / ADB 등. 사망 종류 별 자체 `rate_table` (보통 mortality table 가리킴) |
...
값은 **반드시 위 5 개 중 하나**. 다른 값은 reader error. 새 패턴이 필요하면
engine 의 새 `BenefitPattern` 멤버 추가 + projection kernel 의 routing branch
추가 작업 (library maintainer 의 영역).

In-force 감쇠 (decrement) 는 항상 `Assumptions.mortality_annual` 한 곳에서
모든 상품에 적용됩니다 — 사망 보장이 등록되어 있든 아니든, 사람이 죽으면
계약이 종료되니까. 사망 보장 (DEATH 패턴 coverage) 의 지급률은 그 담보의
