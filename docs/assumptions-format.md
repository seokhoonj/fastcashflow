...
> (담보별 산출방식) 에 있습니다 — `assumptions.xlsx` 는 actuarial basis 만,
...
**중요: `calculation_method` 값이 engine 의 cash flow 산출방식을 결정합니다.**
사용자는 각 담보가 어떤 `calculation_method` 인지 매핑에서 선언만 하고,
engine 이 그 산출방식 (몇 회 지급 / depleting pool / inforce 곱 etc.)
을 hardcoded 로 적용.

| `calculation_method` | engine 산출방식 | 사용 case |
|---|---|---|
...
값은 **반드시 위 5 개 중 하나**. 다른 값은 reader error. 새 산출방식이 필요하면
...
계약이 종료되니까. 사망 보장 (DEATH 산출방식 coverage) 의 지급률은 그 담보의
