...
- `riders` — 특약 마스터. `product`(상품), `rider_code`(특약코드),
  `rider_name`(특약명), `type` 네 열이고 특약 하나에 한 줄씩입니다. `type`은
...
`mortality`도 같은 long-form(`sex,age,rate`)이고, `lapse`는 두
열(`duration,rate`)입니다.
...
9장까지 IFRS 17 측정을 개념부터 실무까지 다뤘습니다. 다음 장은 본문에서
한 걸음 비켜서서, 이 엔진이 왜 그렇게 빠른지 그리고 그 속도가 무엇을
대가로 했는지를 봅니다.
