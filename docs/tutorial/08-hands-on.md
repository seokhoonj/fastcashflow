...
model_points = fcf.ModelPoints.single(
    ...
    premium     = 150,                # 월납 보험료 150
    term_months = 2,                  # 보험기간 2개월
    calculation_methods = {"DEATH": fcf.CalculationMethod.DEATH},
)
```

가입연령 40세, 사망보험금 12,000, 월 보험료 150, 보험기간 2개월 —
5~7장의 그 계약입니다. `12_000`의 밑줄은 자릿수를 읽기 쉽게 나눈
것일 뿐, `12000`과 같습니다. `benefits={"DEATH": 12_000}`은 위에서 등록한
`"DEATH"` 보장에 12,000을 매단다는 뜻입니다. **benefits 의 키는 담보
코드**입니다 — 등록 순서를 외울 필요가 없어 또렷하고, 담보가 여럿일 때
코드로 정렬되어 안전합니다.
...
이 숫자들을 알아보시겠습니까? 5장에서 손으로 구한 BEL -60.15,
6장에서 구한 RA 16.01입니다. 손계산은 할인계수를 반올림해 썼으니
끝자리만 미세하게 다를 뿐, 엔진이 내놓은 -60.15와 16.03은 같은
값입니다. FCF = BEL + RA가 음수라 7.1절에서 본 대로 **이익이 예상되는
계약** — CSM은 44.12, 7.4절에서 손으로 굴리기 시작한 그 값(44.14)이고,
손실요소는 0입니다. CSM 궤적은 `m.csm_path`에 들어 있습니다 — 7.4절의
굴림 그대로 44.12 → 22.06 → 0으로 풀려 갑니다.
...
마지막으로 7.1절의 반대쪽 분기도 확인해 봅니다. 같은 계약에서
**보험료만 100으로** 낮추면 어떻게 되는지요.

```python
# 같은 계약, 보험료만 100 으로 -- 7.1절의 손실부담 분기
cheap = fcf.ModelPoints.single(
    issue_age   = 40,                 # 가입연령 40세
    sex         = 0,                  # 성별 (0=남, 1=여)
    benefits    = {"DEATH": 12_000},  # 사망보험금 12,000 (그대로)
    premium     = 100,                # 월납 보험료 150 -> 100
    term_months = 2,                  # 보험기간 2개월 (그대로)
    calculation_methods = {"DEATH": fcf.CalculationMethod.DEATH},
)
m2 = fcf.gmm.measure(cheap, basis)
print(m2.bel[0])             # BEL
print(m2.csm[0])             # CSM
print(m2.loss_component[0])  # 손실요소
```
...
BEL이 +39.11로 뒤집혔습니다(RA는 보험금 쪽만 보므로 16.03 그대로).
FCF = BEL + RA가 양수가 되어 7.1절에서 본 **손실부담계약**입니다 —
CSM은 0, 예상 손실 55.14는 손실요소로 즉시 인식합니다. 보험료
하나로 `max(0, -FCF)`의 두 분기 — 이익은 CSM에 담아 나중에, 손실은
지금 — 가 갈리는 것을 엔진에서도 그대로 확인한 셈입니다.
