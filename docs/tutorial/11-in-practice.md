...
- 분기 결산 정산 → 결산팩(close pack) 엑셀 파일 생성
- 메모리를 넘는 대규모 포트폴리오
:::
...
recons, movements, group_labels = [], [], []             # 보험계약집합별 정산 결과
for key, segment_basis in basis.segments.items():        # settle 은 세그먼트(단일 Basis) 단위
    ...
    if len(idx) == 0:
        continue
    mv = fcf.gmm.settle(  # 문단 44 기초 -> 기말 정산 -- 일반모형(GMM)
    ...
    movements.append(mv)                   # per-MP 상세 (상세 파일용)
...
- `samples.export` — 패키지 내장 샘플 파일 한 세트를 디스크에 떨굽니다. Excel /
  ...
  자기 파일이 있다고 보면 됩니다.
- `read_basis` — 가정 엑셀을 읽어 `(product, channel)` → `Basis`
  매핑인 `BasisRouter` 로 돌려줍니다. 한 워크북에 여러 세그먼트
...
정산 reconciliation 의 집계 명세를 만들고 (측정 재계산이 아니라 집계),
`write_close_pack` 이 그 명세를 여러 시트의 엑셀 한 권으로 떨굽니다 (감사 추적용
line_code / IFRS17 문단 anchor 포함). 모델포인트 단위 movement 는 엑셀 행 한계를
넘기 쉬워, `movements=` 로 넘기면 본 워크북 옆에 **parquet 상세 파일** (본 파일
옆에 따로 두는 보조 파일) 로 분리해 씁니다.
...
fcf.write_close_pack(package, "samples/close_pack_2026Q1.xlsx",
                     movements=movements)  # 엑셀 + per-MP parquet 상세 파일
...
(`settle_aggregate` / `settle_stream`)·단기 계약의 `paa.settle` 은 쿡북
