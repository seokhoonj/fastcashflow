...
- `coverage` — 담보별 산출방식 (`calculation_methods.csv`) 에 등록된 담보
...
담보 코드를 산출방식으로, `basis` 는 그 코드를 위험률 / 할인 / 사업비로
잇습니다:

```{mermaid}
flowchart TB
    POL["policies.csv (계약 spec)"]
    COV["coverages.csv (담보 가입금액)"]
    CM["calculation_methods.csv (코드 → 산출방식)"]
    B["basis.xlsx (위험률 · 할인 · 사업비)"]
    POL -->|mp_id| MP["모델포인트"]
    COV -->|mp_id| MP
    CM --> MP
    MP --> ENG["엔진 평가"]
