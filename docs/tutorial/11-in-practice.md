...
코드를 어느 *위험률 테이블* 에 잇는지 한 줄씩 적습니다 — 담보 패턴
매핑이 *코드 → 패턴* 매핑이라면 이쪽은 *코드 → 위험률* 매핑.
...
- `coverage_code` — 담보 패턴 매핑 (`benefit_patterns.csv`) 에 등록된 담보
...
benefit_patterns.csv  ──┘   담보 패턴 매핑 (코드 → 패턴 매핑)
                        │
                        ▼
              엔진 평가 (BEL / RA / CSM)
```
...
담보 패턴 매핑에서, *위험률* (실제 숫자) 은 가정 워크북의 `coverages`
시트에서. 한 자리에 모으지 않고 분리한 이유는 두 매핑이 다른 일을
하기 때문 — 담보 패턴 매핑은 "어느 알고리즘을 쓸지", 가정은 "어떤 숫자
값을 넣을지".
