...
코드를 어느 *위험률 테이블* 에 잇는지 한 줄씩 적습니다 — 담보 계산방식이
...
- `coverage_code` — 담보 계산방식 (`calculation_methods.csv`) 에 등록된 담보
...
### 담보 계산방식 — `calculation_methods.csv`
...
calculation_methods.csv  ──┘   담보 계산방식 (코드 → 패턴 매핑)
                           │
                           ▼
                 엔진 평가 (BEL / RA / CSM)
```
...
담보 계산방식에서, *위험률* (실제 숫자) 은 가정 워크북의 `coverages`
시트에서. 한 자리에 모으지 않고 분리한 이유는 두 매핑이 다른 일을
하기 때문 — 담보 계산방식은 "어느 알고리즘을 쓸지", 가정은 "어떤 숫자
값을 넣을지".
...
​    `model_points = fcf.read_model_points("new_business.csv", assumptions, coverages=..., calculation_methods=...)`
