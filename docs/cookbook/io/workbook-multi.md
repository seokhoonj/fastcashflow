...
- segment 마다 다른 **Model** 을 지정하는 `state_machine` 열
- 라우팅 키 매칭 (`product` / `channel`) 과 흔한 키 불일치 함정
:::

[7.1](workbook-single) 은 워크북 한 바퀴와 한 segment 를 들여다봤습니다. 실무
...
적용하는 것과 달리, **`fcf.gmm.measure(mp, basis)`** 는 `basis`
가 dict 일 때 각 모델포인트의 `(product, channel)` 를 보고 사전
에서 맞는 `Basis` 를 골라 적용합니다. dict 라우팅은 `full=True` (각 세그먼트
궤적까지) 와 `full=False` (headline 만, 빠른 경로) 모두 됩니다. `basis` 는
[7.1](workbook-single) 의 `read_basis` 가 돌려준
...
값은 `_DEFAULTS` 행에 두고, 다른 부분만 segment 행이 덮어씁니다 (6.1 의
`_DEFAULTS` 참조). 견본 워크북의 `segments` 는 이렇게 짜여 있습니다:
...
* - `_DEFAULTS`
  - —
  - —
  - MORTALITY_STD / DISCOUNT_STD / state_machine=ACTIVE_WAIVER / ra_confidence=0.75
* - (TERM_LIFE_A, FC)
  - LAPSE_TERM_FC
  - EXPENSE_TERM_FC
  - (defaults 상속)
...
- **해지율은 상품×채널별** — 정기는 `LAPSE_TERM_FC` / `LAPSE_TERM_GA`, 건강은
  `LAPSE_HEALTH_FC` / `LAPSE_HEALTH_GA` 처럼 상품마다 채널별 표를 따로 둡니다
  (TM 은 GA 표를 재사용). 상품과 채널이 함께 해지 행태를 가른다는 실무 구조입니다.
- **사업비는 상품×채널별** — `EXPENSE_TERM_FC` (정기·FC) / `EXPENSE_HEALTH_TM` (건강·TM) 처럼
  상품과 채널 조합마다 다른 `expense_table` 을 가리킵니다.
- **사망률 · 할인율 · state_machine 은 공통** — `_DEFAULTS` 에서 상속해 모든
  segment 가 `MORTALITY_STD` / `DISCOUNT_STD` / `ACTIVE_WAIVER` 를 씁니다.

이 한 줄씩의 차이가 위 출력의 segment 별 BEL 격차로 나타납니다.

## segment 마다 다른 Model

`segments` 시트의 **`state_machine` 열** 은 그 segment 가 쓸 상태 모델을
`Model.from_preset` 레지스트리 이름으로 고릅니다. 한 워크북 안에서 segment 마다
...
* - `ACTIVE_WAIVER`
  - `Model.from_preset("ACTIVE_WAIVER")` — active / waiver 2-state ([3.1](../markov/waiver))
* - `ACTIVE_WAIVER_PAIDUP`
...
[4 장](../semi-markov/reincidence) 처럼 코드로 `Model` 을 직접 조립해야
하므로, 워크북 `state_machine` 열로는 고를 수 없습니다.

## 라우팅 메커니즘과 함정

### 키 매칭 — product / channel

`measure` 는 각 모델포인트의 `(product, channel)` 로
`basis` 사전을 조회합니다. 두 쪽의 코드 문자열이 **정확히** 같아야 합니다 —
`policies.csv` 의 `product` 와 `segments` 시트의 `product` 가
한 글자도 다르면 그 계약은 갈 곳이 없습니다.

:::{admonition} 한글 코드와 유니코드 정규화
:class: note

코드에 한글을 써도 됩니다 (`product = "종신_A"` 등). `measure`
는 양쪽 키를 **NFC 정규화** (같은 글자의 서로 다른 유니코드 분해형을 한 형태로
맞춤) 한 뒤 비교하므로, 파일 인코딩에 따라 분해형이 달라도 매칭됩니다.
:::

### 함정 1 — product / channel 가 없음

BasisRouter 라우팅은 모델포인트에 두 코드가 모두 채워져 있어야 합니다. 둘
