...
- 여러 `(product_code, channel_code)` 가 섞인 portfolio 를
  **`value_segmented` 로 한 번에** 평가 — 각 계약을 자기 segment 가정으로 라우팅
...
## value_segmented — 행마다 맞는 가정으로

`fcf.value(mp, asmp)` 가 한 가정을 전체에 적용하는 것과 달리,
**`fcf.value_segmented(mp, basis)`** 는 각 모델포인트의
`(product_code, channel_code)` 를 보고 `basis` 사전에서 맞는 `Assumptions` 를
골라 적용합니다. `basis` 는 [6.1](workbook-single) 의 `read_assumptions` 가
돌려준 바로 그 `(product, channel) -> Assumptions` 사전입니다.
...
with tempfile.TemporaryDirectory() as tmp:
    tmp = Path(tmp)
    asmp_path = fcf.save_sample_assumptions(tmp / "assumptions.xlsx")
    ...
    basis = fcf.read_assumptions(asmp_path)             # 7 segment 가정 사전
...
`value_segmented` 는 각 모델포인트의 `(product_code, channel_code)` 로
...
코드에 한글을 써도 됩니다 (`product_code = "종신_A"` 등). `value_segmented`
...
`value_segmented` 는 모델포인트에 두 코드가 모두 채워져 있어야 합니다. 둘 중
하나라도 비면 라우팅을 못 합니다. 단일 가정으로 전체를 볼 거라면
`value_segmented` 대신 `value(mp, asmp)` 를 쓰세요.
