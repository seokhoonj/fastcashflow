...
- 원수계약과 재보험을 동시에 풀어, 재보험 BEL이 직접 부채의 "대칭" (부호 반대,
  크기 비례) 이 되는 것을 손계산으로 확인 (둘 다 `-19.90`)
- 보유 재보험계약의 IFRS 17 두 가지 수정 — **전가위험 (RA)** 과
  **순원가 / 이익 (CSM)**, 그리고 손실요소가 없는 이유
- 출재비율 (cession) 을 바꿀 때 결과가 선형으로 움직이는 것
- 포트폴리오에 적용할 때 토이의 대칭 관계가 깨지는 이유 (사업비는 출재 안 됨)
...
- **RA 는 전가위험** (문단 64) — 원수계약의 RA가 원수사가 보유 한
  불확실성이라면, 재보험의 RA는 재보험사로 넘긴 위험의 마진입니다.
- **CSM 은 미실현이익이 아니라 커버의 순원가 / 이익** (문단 65) —
...
0 으로** 둔 것뿐입니다 (할인을 빼면 재보험이 직접 부채와 정확히 대칭이
되는 관계가 깔끔하게 보입니다).
...
- **BEL = 99.50 − 119.40 = −19.90** (= −0.5 × 39.80, 직접 부채와 대칭)
...
basis = fcf.Basis(
    ...
    discount_annual  = 0.0,         # 할인율 0 (대칭 관계를 깨끗하게)
...
대칭이 정확히 됩니다. `cession=0` 이면 출재가 없으니 세 값 모두 0.

### 포트폴리오에 적용 — 그리고 대칭이 깨지는 이유
...
여기서는 토이의 대칭 관계가 **깨집니다** — 직접 BEL은 음수
...
- **할인 timing.** 재보험료는 월초 (`discount_factor_bom`), 회수금은 월중
  (`discount_factor_mid`) 으로 할인되어 토이의 할인 0 가정과 달라집니다.

즉 **재보험 측정은 출재된 보험금·보험료만 보지, 원수계약의 사업비나
onerous 여부를 그대로 따르지 않습니다.** 대칭 관계는 할인 0 · 사업비 0
인 토이의 산물입니다.
