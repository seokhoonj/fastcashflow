...
원 단위로 재현**됩니다. `fcf.solvency.aggregate_required_capital` 이 표3 상관집계 (생명장기·시장·신용
...
basic = fcf.solvency.aggregate_required_capital(ins, mkt, cr, regime=fcf.solvency.KICS, operational=op)
div = (ins + mkt + cr) - fcf.solvency.aggregate_required_capital(ins, mkt, cr, regime=fcf.solvency.KICS)
