...
- 직전 분기 CSM 을 carry-forward 하는 결산 측정 — `gmm.measure_inforce`
- 산출기초 (가정) 가 세그먼트별로 다를 때 — `state.subset` 으로 세그먼트별 측정
- 결산일 시점 BEL / RA / CSM 을 포트폴리오 합계로 읽기
```
...
* - `gmm.measure_inforce`
  - 결산 측정. `state` 에서 `prior_csm` / `lock_in_rate` 을 꺼내, 결산일
    시점 BEL / RA / CSM 을 내고 직전 분기 CSM 을 carry-forward.
```
...
- **CSM (보험계약마진)** — 결산의 핵심. 직전 분기 종가 (`prior_csm`) 를
  ...
  기간 제공한 보장만큼 환입됩니다. 위에서 기말 CSM (547,140) 이 직전 분기
  기초 (562,000) 보다 줄어든 것은, 이번 분기 이자부리보다 환입이 컸기
  때문입니다. **무엇이 얼마씩 움직였는지** 의 항별 분해는 다음 챕터
  ([변동분해](movement)) 에서 풉니다.
- **직전 분기 CSM 이 결산에 들어오는 이유** — IFRS 17 Sec. 44 의
  carry-forward. CSM 은 매 분기 재계산되는 값이 아니라, 직전 종가에서
...
- **`lock_in_rate` 은 가입 시점 율** — 현재 할인율이 아닙니다. 마감파일이
  ...
  혼동하면 CSM 이자부리가 틀립니다.
- **mp_id 정렬** — `apply_inforce_state` 는 spec 과 state 가 같은 순서로
  정렬되어 있다고 가정합니다 (행 i 의 계약 = state 행 i). ETL 에서 두
  파일을 `mp_id` 로 정렬해 두세요.
- **검증** — 한 계약의 결산 CSM 흐름을 손으로 확인하려면
  [검증 패턴](validation) 의 `gmm.trace_csm_step` 으로 직전 CSM ->
  이자부리 -> 환입 -> 기말 CSM 을 항별로 펼쳐 봅니다.

## 인접 레시피

- [변동분해](movement) — 결산 사이에 BEL / CSM 이 움직인 이유를 가정변경 /
  경험 / 이자 / 상각으로 귀속.
