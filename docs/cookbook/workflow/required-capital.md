...
s = fcf.required_capital(mp, basis, regime=fcf.solvency.SII)
...
```python
k = fcf.required_capital(mp, basis, regime=fcf.solvency.KICS)
...
kc = fcf.required_capital(mp, basis, regime=fcf.solvency.KICS, catastrophe=cat)
...
pk = fcf.required_capital(pmp, pbasis, regime=fcf.solvency.KICS, property_codes=("FIRE",))
