...
`BasisRouter` 일 때 — `read_basis()` / `samples.basis()` 가
...
mp_one = ModelPoints(
    ...
    benefits            = {"DEATH": np.array([100_000_000.0])},    # DEATH 보장의 보험금 1 억
...
BEL이 +16.63% 움직였습니다. 이 16% 가 어디서 왔는지 위쪽 섹션이
설명해 줍니다:
...
- **BEL deltas (key months)** — `BEL[0]` 의 +16.63% 가 `BEL[240]` 의
  -0.38% (만기금이 줄어든 효과) 와 합쳐져 만들어진 결과.

`+16.63%` 라는 숫자 하나가 아니라 **각 단계가 어떻게 얽혔는지** 가
