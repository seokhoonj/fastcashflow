...
def basis(*, settlement=None):
    ...
    return Basis(
        ...
        lapse_annual=lambda s, ia, d: np.full(d.shape, 0.05),
...
def book(b, *, prior_csm=0.0, lc_open=0.0,
    ...
    prior_count  = 1000.0 * surv[em_open]                       # 기초 잔존 (1,000 건 책)
    ...
    state = InforceState(
        ...
        lock_in_rate=0.03, prior_count=np.array([prior_count]),
...
손실부담 책 (직전 CSM = 0, 손실요소를 운반) 은 보장이 제공되면서 **손실요소를
...
```python
# 해지환급금 (투자요소) 이 있는 책. 기대 (~1,166,843) 보다 33,157 더 지급
b_surr = Basis(
    mortality_annual=death,
    lapse_annual=lambda s, ia, d: np.full(d.shape, 0.05),
    discount_annual=0.03, ra_confidence=0.75, mortality_cv=0.10,
    coverages=(CoverageRate("DEATH", death),),
