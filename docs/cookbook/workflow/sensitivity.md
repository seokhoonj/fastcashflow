...
- `show_trace_diff` 로 한 계약의 shock 전파 (rate -> cash flow -> BEL -> CSM)
  를 ASCII 트리로
...
for factor in [1.00, 1.05, 1.10, 1.25, 1.50]:
    v = fcf.value(mp, basis_with_mortality(shock_mortality(factor)))
...
## 한 계약 들여다보기 — show_trace_diff

sweep 은 *얼마나* 변하는지를 보여주지만, *어디서* 그 변화가 생기는지는
`show_trace_diff` 가 두 basis 를 나란히 놓아 보여줍니다 — rate 부터 cash
flow, BEL, CSM 까지 한 줄씩.

```python
base    = basis_with_mortality(death_fn)
shock10 = basis_with_mortality(shock_mortality(1.10))
fcf.show_trace_diff(0, mp, base, shock10, label_a="baseline", label_b="mort+10%")
```
...
`show_trace_diff` 는 동일한 항목은 숨기고 *바뀐* 것만 보여주므로, shock 이
...
변수에 lift 한 뒤 바꿔 끼우고 `value` 를 다시 부르면 됩니다.

### 여러 가정 동시 (시나리오)

여러 가정을 한꺼번에 바꾸면 *시나리오* 가 됩니다 (예: 사망률 +10% **와**
해지 -20% 동시). `Assumptions` 를 그 조합으로 한 벌 만들어 평가하면 됩니다 —
단일 가정 민감도와 달리 cross effect (상호작용) 까지 반영됩니다.

### segment 별 shock

[6.2](../io/workbook-multi) 의 `basis` 사전을 통째로 흔들 수도 있습니다 —
각 segment 의 `Assumptions` 를 shock 한 새 사전을 만들어 `value_segmented`
...
- [7.2 검증 패턴](validation) — `show_trace` 로 한 계약의 BEL / CSM 계산
  경로를 한 줄씩. `show_trace_diff` 의 단일 basis 판.
- [1.3 사망률의 두 역할](../basics/mortality-roles) — 사망률이 감쇠와 청구
  두 자리에 들어가는 구조. shock 을 양쪽에 먹이는 이유.
- [6.2 워크북 — 다 segment](../io/workbook-multi) — `value_segmented` 로
  portfolio 전체 shock.
