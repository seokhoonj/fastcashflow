...
(`CSM_0 = max(0, -(BEL + RA))`, IFRS 17 문단 38).
...
- **factor 1.25**: CSM이 0 으로 **소진** 되고, FCF가 양수로 넘어가
  **loss component 3,201** 이 나타납니다 — 계약이 onerous (손실부담) 로
  뒤집혀 그 손실이 즉시 인식됩니다 (문단 47-49). CSM은 음수가 못 되므로
...
- [8.2 검증 패턴](validation) — `gmm.trace` 로 한 계약의 BEL / CSM 계산
  경로를 한 줄씩. `gmm.trace_diff` 의 단일 basis 판.
- [1.3 탈퇴와 발생](../basics/mortality-roles) — 단순 사망보험에서 같은
  사망률을 탈퇴와 발생 두 자리에 줄 때. shock 을 양쪽에 먹이는 이유.
- [7.2 워크북 — 다중 segment](../io/workbook-multi) — `measure` 로
  portfolio 전체 shock.
