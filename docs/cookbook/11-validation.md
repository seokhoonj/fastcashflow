# 11장. 검증 패턴 — show_trace 4종으로 신뢰 빌드업

```{admonition} 이 챕터에서 배우는 것
:class: tip

- 한 계약의 BEL / RA / CSM 이 **어떤 테이블, 어떤 rate, 어떤 cash flow**
  로 계산되는지 추적하기 — `show_trace`
...
이 챕터는 자사 데이터로 평가를 돌려보고 **결과를 신뢰** 하기 위한
...
평가 엔진은 정의상 **블랙박스** 입니다. {{ fcf }} 도 예외가 아닙니다.
한 줄의 `value()` 호출이 수십 개의 테이블 lookup, 월별 cash flow 산출,
backward / forward recursion 을 거쳐 BEL / RA / CSM 하나의 숫자로
요약됩니다. 그 숫자를 사내 계리위원회 / 감독당국 / 회계감사인에게
설명해야 할 때, 그리고 자사 손계산 결과와 어긋날 때, **어디서 어떻게
계산됐는지 한눈에 보이는 도구** 가 필요합니다.

전형적인 검증 시나리오는 셋입니다:

1. **신규 도입** — 처음 자사 데이터를 흘리고 BEL 값을 받았는데,
   사내 별도 산출치와 X% 차이. 어디서 어긋났는가?
2. **분기 결산** — 직전 분기 대비 BEL 이 Y% 움직였는데, 어느 가정 /
   어느 segment 가 주범인가?
3. **시나리오 / 민감도** — "사망률을 10% 올리면 BEL 이 얼마나 움직이나"
   — 결과 숫자만 보지 말고 **어디서 어떻게** 변했는지 추적.

세 시나리오 모두 같은 출발점에서 풉니다: **단일 계약 (one model point) 의
계산 경로 전체** 를 보는 것입니다. 포트폴리오 전체를 한꺼번에 보면 노이즈
가 너무 많아 원인이 가려집니다. {{ fcf }} 의 검증 도구 4종은 **한 계약**
에 초점을 맞춥니다.
...
* - `show_trace_diff`
  - 두 가정 (basis) 비교 — 바뀐 테이블 / 연도별 rate 변화 / cash flow 의
    propagation / BEL / RA / CSM 의 절대·% 변화
  - 시나리오 / 민감도. mortality +10% 가 BEL 에 14.2%, RA 에 9.8% 라면
    각각 어디서 왔는지
```
...
- **Assumptions / Coverages** — "엔진이 내가 의도한 테이블을 잡았나?"
...
## 11.4 결과 해석 — 트리의 핵심 섹션

### 11.4.1 Assumptions 블록의 `_fcf_table_id`

`mortality_annual -> MORTALITY_STD` 형태로 표시된 부분은 rate 함수가
**어느 워크북 시트의 어느 table_id** 에서 왔는지 알려줍니다.
`read_assumptions()` 가 rate 함수에 `_fcf_table_id` 메타데이터를
...
이 줄어듭니다. 가능하면 워크북 / `read_assumptions()` 경로로 통일하는
편이 검증에 유리합니다.

### 11.4.2 Coverages 블록의 risk 와 is_diagnosis

각 rider 행의 `risk` 는 RA (Risk Adjustment = 위험조정) 계산 시 어느
변동계수 (`mortality_cv` / `morbidity_cv`) 와 묶이는지 결정합니다.
`is_diagnosis=True` 인 rider 는 진단 시 한 번만 지급되고 in-force 의
"미진단 풀" 이 줄어드는 형태입니다 (CANCER 행). False 면 재발 가능한
형태 (INPATIENT 행 — 입원 발생).

### 11.4.3 Rates 블록의 axes 라인
...
샘플 트레이스의 CSM 트랙이 거의 비어 있게 보입니다. 자사 데이터로
바꾸면 보통은 `CSM > 0` 가 정상 흐름입니다.

## 11.5 자주 쓰는 변형

### 11.5.1 월별 BEL 식 전개 — show_bel_step

`show_trace` 가 BEL 의 **궤적** 을 anchor 월에서 값으로 보여준다면,
`show_bel_step` 은 한 달의 BEL 식을 **항별로** 풀어 보여줍니다.
...
fcf.show_bel_step(0, mp, basis, months=[0, 12, 239, 240])
```
...
### 11.5.2 월별 CSM 식 전개 — show_csm_step

CSM 은 BEL 의 반대 방향 — **forward recursion** 입니다. 시작값
`csm[0] = max(0, -(BEL+RA))` 에서 출발해 매월 이자부리 + coverage_unit
환입.

```python
fcf.show_csm_step(0, mp, basis, months=[1, 60, 120, 240])
```

샘플의 정기보험은 손실부담이라 모든 step 에서 `csm = 0`. 의미는
"floor 가 작동" — 차라리 가입 시 CSM 이 음수가 될 것을 0 으로 막은 것.
...
### 11.5.3 가정 변경의 전파 추적 — show_trace_diff

"사망률 +10% 면 BEL 이 얼마나 움직이나" 는 시나리오 / 민감도의 가장
...
fcf.show_trace_diff(0, mp, baseline, shocked,
...
- **Rate deltas** — 매년 mortality 만 정확히 +10.00%. 다른 rate (lapse,
  waiver, rider) 는 변화 없음 (출력에서 자동 숨김).
...
보이는 게 핵심입니다. 사내 검토 시 "왜 BEL 이 14% 움직였는지" 의
질문에 일직선으로 답할 수 있습니다.

## 11.6 함정 — 검증 시 자주 마주치는 것

### 함정 1 — `residual` 이 0 인데 손계산과 안 맞음

`show_bel_step` 의 잔차는 **엔진의 식 vs 엔진 자신** 의 비교입니다.
...
3. **A/E factor / improvement / age_shift 누락** — 엔진은 적용했는데
   손계산은 raw rate 만 썼을 가능성. `show_trace` 의 Assumptions 행에
...
`show_trace` 의 Assumptions 블록은 검증에 필수: 자신이 단순화한 항목이
실제로 0 인지 (예: `gamma_flat = 0`) 확인합니다.

### 함정 3 — shock 이 의도와 다르게 전파됨

`show_trace_diff` 에서 mortality 만 올렸는데 lapse / expense / surrender
...
포트폴리오에서 `show_trace(0, mp, basis)` 를 호출해도 단일 계약 측정
...
fcf.show_trace(int(idx), mp, basis)
```
...
- **10장 시나리오 / 민감도 분석** — `show_trace_diff` 의 활용을
  단일 계약 검증 너머 포트폴리오 단위 민감도로 확장.
- **1장 정기보험 평가 1.6 검증 절** — 2 개월 손계산 패턴. 이 챕터의
  도구로 그 검증을 다시 추적하면 식이 어떻게 매핑되는지 명확.
- **9장 Excel 워크북 — 다 segment** — `show_trace` 의 segment 라우팅이
  맞는지 확인하는 자리.

기본 튜토리얼의 5장 (BEL 계산) / 7장 (CSM 계산) 이 `show_bel_step` /
`show_csm_step` 이 풀어 보여주는 식의 derivation (유도) 을 다룹니다.

```{admonition} 검증으로 무엇이 보장되고 무엇이 안 보장되나
:class: warning

이 챕터의 도구들은 **엔진이 출력된 식을 정확히 따른다** 는 것을
보여줍니다 (residual ≈ 0). 식 자체가 자사 상품 / 자사 가정 / IFRS 17
의 의도에 맞는지는 **사용자 판단** 입니다.

검증으로 잡히는 것: 입력 / 출력 / 식의 매핑이 끊기지 않았다는 사실.

검증으로 안 잡히는 것: 입력 가정이 옳은지, 식의 선택이 자사 상황에
적합한지, IFRS 17 의 §B71 mid-month 가정이 자사 회계정책과 정합한지.
이 판단은 도구 너머의 영역입니다.
```
