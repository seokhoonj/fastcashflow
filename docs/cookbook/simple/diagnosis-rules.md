...
cancer_rate = 1 - (1 - 0.10) ** 12

# 산출기초
basis = fcf.Basis(
    mortality_annual = 0.0,   # 보유계약 사망률 0 (진단 풀에 집중)
    ...
    coverages        = (
        fcf.CoverageRate("CANCER", cancer_rate),  # 암진단 1종 (청구 rate = cancer_rate)
    ),
)
...
바꿀 뿐, 진단으로 풀이 줄어드는 움직임은 건드리지 않습니다. 즉 면책기간에
...
basis = fcf.Basis(
    ...
    morbidity_cv     = 0.0,
    coverages        = (
        fcf.CoverageRate("CANCER",   cancer_rate),    # 암진단
        fcf.CoverageRate("CEREBRAL", cerebral_rate),  # 뇌혈관 진단
    ),
)
...
- [1.4 담보별 산출방법](../basics/coverage-mechanics) — DIAGNOSIS 의
  `undiagnosed` 풀 움직임. 면책 / 감액이 풀을 안 건드리는 이유의 근거.
