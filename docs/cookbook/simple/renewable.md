...
- **IFRS 17 계약의 경계 (Sec. 34)** — 어디까지가 **현재 계약**의 현금흐름인지
...
되는가?"** 입니다. 답은 **계약의 경계 (contract boundary, Sec. 34)** 가 정합니다 —
...
Sec. B62 의 "substantive right" 판단은 약관 · 재가격권 · 감독해석에 따라
...
* - `ModelPoints.contract_boundary_months`
  - **Sec. 34 계약의 경계** — 측정이 멈추는 월. 미설정 시 `term_months` (경계
...
재산정할 수 있으면 **차기갱신** (Sec. 34), 못 하면 (갱신가가 약관표로 고정 등)
**최종만기** 쪽입니다.
...
**예측 불가** 합니다. 그래서 Sec. 34 가 차기갱신에서 경계를 끊는 것이고, 갱신
