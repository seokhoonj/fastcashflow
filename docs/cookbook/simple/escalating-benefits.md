...
```{admonition} 체증은 보험료도 올린다
:class: note

보험금이 오르면 그만큼 BEL이 커지고, 가격(보험료)도 동반해 올라야 합니다.
이 챕터는 **측정** (주어진 체증 구조의 부채) 에 집중합니다. 체증 보험금에
맞춰 보험료를 **푸는** 쪽은 `fcf.solve_premium` 이고, 보험료 자체를 시간축에서
키우는 **체증형 보험료** 는 `Basis` 의 `premium_factor_annual` (경과연수
-> 보험료 배수) 로 — 본 챕터의 보험금 체증과 짝을 이루는 보험료측 도구입니다.
```
...
### 함정 2 — 체증을 `annual_to_monthly` 로 거는 줄 착각
