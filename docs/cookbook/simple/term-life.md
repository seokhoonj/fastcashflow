...
# 사망률 -- 월 사망률 1% 의 연 환산 (모든 sex/age/duration 에 동일)
...
`loss_component` 로 잡힙니다 (IFRS 17 문단 47).
...
portfolio = fcf.ModelPoints(
    ...
    benefits            = {"DEATH": rng.integers(10, 100, n_contracts) * 1_000_000},  # 1 ~ 10억
...
mp = fcf.ModelPoints.single(
    issue_age           = 40,                      # 가입연령
    benefits            = {"DEATH": 100_000_000},  # 사망보험금 1억
...
위 "한 계약 평가" 예제에서 `death_rate` 을 두 자리 (`mortality_annual` 과
