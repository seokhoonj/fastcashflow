...
"유니버설" 은 **보험료가 적립금으로 쌓이고, 위험보험료 (COI, cost of insurance) 가
거기서 차감되고, 공시이율이 붙는** 적립 방식입니다. 무엇을 보장하느냐 (담보) 도, 어떻게 할인하느냐
...
얹어줍니다. 보험사의 진짜 위험은 그 **순보장액** (NAR, net amount at risk)
`= max(0, face - AV)` 뿐이고,
...
- **금리연동형 / 고정 (간접참가) -> `gmm.measure`** — locked-in 할인곡선 (문단 36)
...
## 해지공제 (surrender charge)

실제 유니버설보험은 보통 **해지공제** 를 둡니다 — 초기 해지 시 모집비용 회수를
위해 계좌가치의 일부를 떼고 돌려주며, 보통 가입 초기에 크고 해가 갈수록 0 으로
줄어듭니다. `Basis.surrender_charge_annual` 에 **계약연도별 공제율** 을 주면 (스칼라,
배열, 또는 5-인자 `RateFn`), 해지 지급액이 `max(0, av_mid x (1 - 공제율))` 이 됩니다.
설정하지 않으면 (`None`) 계좌가치 전액을 지급 — 종전과 동일합니다.

공제는 **지급액만** 줄입니다 — 계좌 적립 자체와 해지율은 그대로라, 해지 현금흐름이
정확히 `(1 - 공제율)` 만큼 줄고 그만큼 부채 (BEL) 가 낮아집니다:

```python
import numpy as np
from dataclasses import replace

# 1년차 7%에서 매년 1%p씩 줄어 7년차 0%가 되는 해지공제
sched   = lambda s, a, d, ic, el: np.maximum(0.07 - 0.01 * d, 0.0)
plain   = fcf.gmm.measure(mp, basis)
charged = fcf.gmm.measure(mp, replace(basis, surrender_charge_annual=sched))
print(f"BEL  (해지공제 없음) = {np.sum(plain.bel):>14,.0f}")
print(f"BEL  (해지공제 적용) = {np.sum(charged.bel):>14,.0f}")
```

출력:

```text
BEL  (해지공제 없음) =     -2,648,834
BEL  (해지공제 적용) =     -2,991,606
```

해지 시 떼는 만큼 보험사 유출이 줄어 BEL 이 더 낮아집니다 (더 이익). 해지공제는
[5.3](../variable/crediting-floor) 의 적립이율 보증 시간가치에도 반영됩니다 —
보증이 떠받친 적립도 해지 시엔 공제 후 (`x (1 - 공제율)`) 지급되므로, `vfa.tvog`
의 해지 레그가 그만큼 작아집니다 (사망·만기 레그는 그대로).
