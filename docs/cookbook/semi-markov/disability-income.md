...
* - `State("disabled", benefit=True, sojourn_tracking_months=D, ...)`
  - 장해. `benefit=True` 가 **매월 `disability_income` 지급**, `sojourn_tracking_months > 0`
...
model = StateModel(states=(
    ...
    State("disabled", benefit=True, sojourn_tracking_months=24, transitions=(  # 매월 소득 + 경과 추적
