...
print(f"CSM           = {m.csm[0, 0]:.2f}")               # 계약서비스마진
```
...
asmp_no_excl = replace(asmp,
    ci_reincidence_annual=lambda s, a, d, sd: np.full_like(sd, 1 - (1 - 0.20) ** 12,
                                                           dtype=float))
print(f"seated BEL = {fcf.value(mp_seat, asmp_no_excl).bel[0]:.2f}")   # -> 199000.00
```
...
- 4.2 장해소득보상 (DI) (작성 예정) — 같은 semi-Markov 인프라로 회복률
  (disabled -> active) 을 경과 의존으로.
- [검증 패턴](../workflow/validation) — `show_trace` 로 상태별 · 코호트별
  점유를 한 줄씩 확인.
```
