...
(국가암등록) 의 연령 곡선** 으로 깔고, **재진단율을 그 위에 재발 hazard 배수**
로 얹습니다 — 재발 위험은 1차 진단 후 1~3년에 높고 (면책 직후), 5년이 지나면
신규 원발암 수준으로 가라앉는 임상적 패턴입니다:

```python
import numpy as np

# 계리적 가정 -- 1차 진단 = 공개 KOSIS 국가암등록 구조, 재진단 = 재발 hazard 배수
def ca_incidence(s, a, d):                          # 1차 진단율 (연령 상승, 여성 젊은연령↑)
    base = 0.0010 * np.exp((a - 30.0) * 0.082)
    fem  = np.where(s == 1, 1.0 + np.maximum(0.0, 50 - a) * 0.030, 1.0)
    return base * fem
...
print("1차 30/40/50/60 :", [round(float(ca_incidence(np.array([0]), np.array([a]), 0)[0]), 5)
                           for a in (30, 40, 50, 60)])
print("재발배수 sd6/24/48/72:", [float(np.select([np.array([x]) < 12, np.array([x]) < 36,
       np.array([x]) < 60], [0.0, 1.8, 1.2], default=0.9)[0]) for x in (6, 24, 48, 72)])
```

```text
1차 30/40/50/60 : [0.001, 0.00227, 0.00516, 0.0117]
재발배수 sd6/24/48/72: [0.0, 1.8, 1.2, 0.9]
```
