...
- 변액보험이 보장형 (GMM) 과 다른 <u>측정 모델</u> (VFA) 로 평가되는 이유 —
  계좌가치가 굴러가고 보험사는 <u>수수료</u> 를 번다
- `account_value` + 최저보증 (`guaranteed_death_benefit` /
  `guaranteed_accumulation_benefit`) 을 모델 포인트에 거는 자리
- `measure_vfa` 의 결정론 측정 — 보증의 <u>intrinsic value</u> 가 BEL 에 들어가는 모습
- `return_scenarios` 를 넣으면 드러나는 보증의 <u>시간가치 (TVOG)</u> — 단일
...
계좌에서 떼는 <u>수수료</u> 입니다. IFRS 17 은 이런 직접참가 계약을 **VFA
...
보증액을 넘는 <u>초과분</u> (`보증액 - 계좌가치`, 양수일 때) 이 보험사가 부담하는
보증 비용입니다. 계좌가 보증을 웃돌면 초과분은 0 — 보증은 계좌가치에 대한
**풋옵션** 처럼 <u>하락에만</u> 비용이 발생합니다.
...
* - `ModelPoints.guaranteed_accumulation_benefit`
  - GMAB 보증액. 기본 0 = 보증 없음
...
basis = fcf.Assumptions(
    ...
    expense_cv        = 0.10,       # 사업비 변동계수 (VFA 의 RA = 사업비위험)
...
mp = fcf.ModelPoints.single(
    ...
    guaranteed_death_benefit        = 1.02e8,   # GMDB 최저사망보증 (102%)
...
차이 ≈ 0.22M 뿐). 단일 결정론 run 만 보면 "보증이 거의 공짜" 라는 <u>틀린</u>
결론에 이릅니다.
...
- **시간가치가 수수료를 넘으면** CSM 이 0 으로 깎이고 손실요소
...
`return_scenarios` 의 시간가치 패스는 v1 에서 `guaranteed_credit_rate` (적립
보증이율) 가 portfolio 전체에서 동일해야 합니다 — per-MP 로 다른 적립보증률에
...
`guaranteed_credit_rate` 를 0 보다 크게 주면 계좌가 매월 `max(수익률, 보증
...
결정론 run 에는 <u>원리상</u> 보이지 않습니다.

### 함정 3 — 보증액 기본 0

`guaranteed_death_benefit` / `guaranteed_accumulation_benefit` 를 안 주면 0
