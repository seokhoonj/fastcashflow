...
(5.1 의 궤적). 한 경로만 보면 floor 가 작동하는 자리가 한정돼 intrinsic 50,594 만
...
결정론만 보면 보증비용이 5만원 — "거의 공짜" 로 보입니다. 시나리오를
넣어야 6백만의 진짜 비용이 드러납니다. **단일 결정론 측정으로 변액 보증을
평가하면 안 되는** 이유입니다.
:::
...
생성기) 산출물입니다. fastcashflow 는 시나리오를 생성하지 않고, 받아서 보증
...
사망률 0.5% 라는 얇은 확률로 작동하니 시간가치가 훨씬 작습니다 (236,056).
...
중앙 성장 가정에선 floor 가 한정된 구간에서만 작동해 intrinsic 이 작게
나옵니다. 보증의 진짜 비용 (시간가치) 은 `return_scenarios` 를 넣어야
드러납니다 — 단일 결정론 측정에는 원리상 보이지 않습니다.
...
- `vfa.trace(mp_index, mp, basis, return_scenarios=scen)` — 이 계약의 계좌가치
  궤적, floor 가 작동하는 자리, TVOG 까지 트리로 확인 (`return_scenarios` 를 주면
  시간가치 경로도).
