...
## 12.1 왜 `benefit_patterns.csv` 인가 — 라이프사이클 분리
...
|---|---|---|---|
| **DEATH** | 사망 사건 발생 시 amount 지급 | 자체 rate_table (사망률 / 상해사망률 / ADB 등) | 코드 ≥ 1 의 일반 coverage 슬롯 |
| **MORBIDITY** | 사건 발생 시 amount 지급, 다음 달에도 발생 가능 | 자체 incidence rate_table | 코드 ≥ 1 |
| **DIAGNOSIS** | 첫 발생 시 1 회 지급, 그 이후 미지급 | 자체 incidence rate_table | 코드 ≥ 1 (depleting pool) |
...
## 12.4 카탈로그 작성 — `benefit_patterns.csv`

세 컬럼:

```
coverage_code,coverage_name,benefit_pattern
DEATH_MAIN,사망 (주계약),DEATH
ADB,재해사망 특약,DEATH
CA_DIAG,암 진단 특약,DIAGNOSIS
HOSP,입원 일당,MORBIDITY
ANN,종신 연금,ANNUITY
MAT,만기환급,MATURITY
```
...
```{tip}
`coverage_code` 가 `"DEATH_MAIN"` 인 행은 엔진의 **주계약 사망 슬롯**을
가리키는 reserved 코드입니다 — `ModelPoints.death_benefit` 의 amount 가
이 슬롯에 들어가고 rate 는 `Assumptions.mortality_annual` 이 됩니다.
다른 사망 종류 (ADB 등) 는 같은 DEATH 패턴이지만 `coverages.xlsx` 의
일반 rate-driven 자리에 등록됩니다.
```

## 12.5 변형 — 회사 카탈로그를 어떻게 짜는가
...
- {ref}`11장 — 검증 패턴 (show_trace) <cookbook/11-validation>` —
  카탈로그 변경이 어느 계산 단계에 어떻게 들어가는지 추적.
- 1장 — 정기보험 (DEATH 만 사용하는 가장 단순한 사례).
- (작성 예정) 2장 — DIAGNOSIS 추가, depleting pool mechanic.
