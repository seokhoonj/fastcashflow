...
해지율이 낮아지는 쪽을 씁니다. 저축성 spike 의 연결은 아래 **변형**
절에서 다룹니다.
...
두 경로는 **서로 다른 측정 모드** 라 바꿔 쓸 수 없습니다 — 신계약 CSM을
...
* - 자리
  - 무엇
* - `Basis.state_machine`
  - `Model.from_preset("ACTIVE_WAIVER_PAIDUP")` — active / waiver / paidup 3-state 모델
* - `Basis.lapse_annual`
  - active 상태의 해지율 (납입중)
...
세 상태를 그림으로 (paidup 은 전이가 아니라 **seating** 으로만 진입 — 완납
보유계약 평가; active 와 paidup 의 해지율이 다름):

:::{mermaid}
flowchart LR
    START(("신계약")) --> ACT["active<br/>납입중"]
    SEAT(("완납<br/>보유계약")) --> PU["paidup<br/>납입후"]
    ACT -->|"waiver_incidence"| WV["waiver<br/>납입면제"]
    ACT -->|"mortality · lapse"| EXIT(("종료"))
    WV -->|"mortality"| EXIT
    PU -->|"mortality · lapse_paidup"| EXIT
    classDef stock fill:#eaf1f8,stroke:#547fa6,color:#17344e
    classDef step fill:#f7f2e8,stroke:#b38a45,color:#493617
    class ACT,WV,PU stock
    class START,SEAT,EXIT step
:::
...
- 월 할인율 0 (상태/해지 움직임에 집중)
:::

```python
import numpy as np
import fastcashflow as fcf
from fastcashflow import STATE_PAIDUP, STATE_ACTIVE
from fastcashflow.multistate import Model

# rate 함수 -- 모든 rate 는 (sex, issue_age, duration) 시그니처
...
basis = fcf.Basis(
    ...
    state_machine             = Model.from_preset("ACTIVE_WAIVER_PAIDUP"),  # 3-state
    coverages               = (
...
소멸하고, 그만큼 미래 사망보험금이 더 남아 **BEL이 커집니다**
(2,911.49 > 2,684.88).
...
순수하게 부채를 **방출** 하기만 해 해지율이 낮을수록 runoff 가 느려져 BEL이
커집니다 — 위에서 완납 BEL이 더 큰 이유입니다. **저축성·고환급 상품은
방향이 반대** 입니다: 완납 시점에 해약환급금이 정점이라 해지가 환급금
유출을 일으켜 BEL을 **키우고**, 해지율 자체도 완납 시점에 spike 합니다.
...
- 위 두 측정의 유일한 차이는 시작 상태입니다. 같은 사망률 · 같은 보험금
  · 같은 보험료 (0) 인데도 BEL이 226.61 만큼 다른 것은 **전적으로
  해지율** (2% vs 10%) 때문입니다.
...
basis = fcf.Basis(
    mortality_annual = death_rate,  # 보유계약 사망률 (월 1%)
    lapse_annual     = lapse_rate,  # 해지율 (납입중 30% → 납입후 2%)
...
`Model.from_preset("ACTIVE_WAIVER_PAIDUP")` 에는 paidup 을 목적지로 삼는 전이가
...
완납 계약은 보험료를 안 냅니다. `premium` 을 0 으로 두지 않으면
이미 완납된 계약에서 보험료 수입이 잡혀 BEL이 틀립니다. (paidup state
자체도 `pays_premium=False` 라 보험료가 0 으로 계산되지만, 입력에서도 0 으로
두는 것이 의도를 드러냅니다.)
...
- [3.1 보험료 납입면제 (waiver)](waiver) — 2-state 입문. 본 챕터의 직접
  출발점.
- [1.3 탈퇴와 발생](../basics/mortality-roles) — decrement (사망 +
  해지) 와 보장 청구의 분리.
