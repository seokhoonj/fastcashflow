...
지급률 곡선). 3.1 의 상태 전이와는 직교하는 축이라, 상태 없는 계약에도
그대로 적용됩니다.
...
면책 / 감액은 `coverages` (long-form) 파일의 담보 행에 세 컬럼으로 붙습니다.
...
- 룰이 없으면 cash flow 는 `[10,000, 9,000, 8,100, 7,290]` 이고 BEL =
  34,390. 면책 + 감액이 BEL 을 **34,390 → 15,840** 으로 낮춥니다.
...
감액기간은 절반, 그 이후 정상. 이 곡선의 현재가치 합이 BEL 이고, 면책 /
감액이 길수록 / 깊을수록 BEL 이 낮아집니다 (보험사 지급이 줄어드니까).
...
- [2.2 사망 + 단순 진단 일시금](../simple/death-diagnosis) — 면책 / 감액
  없는 진단 담보. 본 챕터의 출발점.
- [1.4 보장 청구 메커니즘](../basics/coverage-mechanics) — DIAGNOSIS 의
  `undiagnosed` 풀 동학. 면책 / 감액이 풀을 안 건드리는 이유의 근거.
- [3.1 보험료 납입면제 (waiver)](waiver) — 상태 축. 면책 / 감액 (담보 룰
  축) 과 직교하며 한 계약에 공존 가능.
- [검증 패턴](../workflow/validation) — `show_trace` 로 담보별 면책 / 감액
  적용 구간을 한 줄씩 확인.
```
