...
│   ├── is_main               · 주계약 표시 (선택, TRUE/FALSE; %가입금액 사업비의 base)
│   ├── amount                · 가입금액 (보험금)
│   ├── premium               · 월 보험료 (선택)
│   ├── waiting               · 면책기간 개월수 (선택)
│   ├── reduction_end         · 감액기간 종료 개월수 (선택)
│   ├── reduction_factor      · 감액기간 중 지급률 (선택, 0..1)
│   └── term                  · 담보 만기 개월수 (선택, 0=계약 boundary 까지)
│
...
│   ├── fcf.gmm.settle(mp, state, basis, period_months=)                 ── 결산: 문단 44 기초 → 기말 정산 (세그먼트 단위)
│   ├── fcf.paa.settle(mp, state, basis, period_months=)                 ── 단기 (PAA) 계약의 결산: 문단 55(b) LRC 정산
