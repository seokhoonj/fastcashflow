...
으로 만듭니다 — LTFR(장기선도금리, long-term forward rate — 위 UFR 의 한국 규제
명칭, 같은 개념)·LP(유동성프리미엄, liquidity premium)·최종관찰만기(LLP, last
liquid point) 일정은 **금융위·금감원 보험개혁회의**의 「보험부채 할인율 현실화
방안」(2024.11~)이 정합니다. 그 **만기수익률→현물 전환과
...
* - `ufr`
  - 장기수렴 선도금리(LTFR/UFR). **금융위·금감원이 일정 공시** — 2023 4.80%에서
    매년 -25bp, 2026년 원화 4.05%
* - `alpha`
  - 수렴 속도. 공시값이 **아니라** 수렴기준으로 산출(EIOPA(유럽 보험·연금 감독기구,
    European Insurance and Occupational Pensions Authority)식: LLP+40년에 forward 가
    UFR ±1bp 이내가 되는 가장 작은 alpha). v1 은 입력으로 받음
:::

:::{admonition} `alpha` 는 어떤 값이 가능한가
:class: note

- **코드 제약** — `smith_wilson` 은 `alpha > 0` 이면 무엇이든 받습니다(`0` 이나
  음수는 `ValueError`). 수학적 상한은 없습니다.
- **실무 범위** — 보통 **0.05 ~ 0.20**. EIOPA 는 **하한 0.05** 를 두고, EUR 약
  0.13 · CHF 약 0.129 처럼 대부분 이 구간에 들어옵니다. 0.3 을 넘으면 LLP 직후
  곧바로 UFR 로 점프해 비현실적입니다.
- **고르는 법** — 수렴을 만족하는 **가장 작은 alpha**(시장 데이터에 최대 가중).
  클수록 UFR 로 빨리, 작을수록 관측금리 근처에 오래 머뭅니다(아래 **검산** 절 참조).
:::
...
줍니다. **UFR(LTFR)·LP 는 금융위·금감원 공시 일정**을 쓰세요. **LLP(최종관찰만기)는
...
- [경제 시나리오 생성 — Hull-White](scenario-generation) — 이 곡선에 캘리브레이션한
  위험중립 시나리오로 TVOG / 확률측정.
