# 1.3 탈퇴와 발생 — 누가 계약에 남고, 어떤 보험금이 나가는가
...
- **탈퇴(decrement)** = 계약 전체 in-force에서 빼는 입력. `mortality_annual`은
  ...
  **율**이지 **력**(force, μ)이 아니고, 엔진은 `inforce x (1 - q)`로 깎습니다.
  `lapse_annual`은 해지탈퇴율.
- **발생(incidence)** = 보험금 줄 사유의 빈도. 담보의 `rate`가 그것. 발생은
  *계약 전체 in-force는 건드리지 않습니다*. 다만 담보에 따라 자기 하위 풀은
  줄일 수 있어 — DIAGNOSIS는 자기 미진단 풀을 `(1 - 발생률)`로 깎고,
  DEATH·MORBIDITY는 아무 풀도 안 깎습니다 (위 표 마지막 열).
