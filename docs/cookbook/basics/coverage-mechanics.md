...
- 손계산 / 엔진 출력 / `show_trace` 의 `undiagnosed` 트리가 어떻게
  서로를 검증하는가
```
...
print(f"BEL[0]        : {float(r.bel[0, 0]):.2f}")            # 최선추정부채
...
```python
import fastcashflow as fcf
fcf.show_trace(0, mp_diagnosis_above, asmp_diagnosis_above)
```
...
## `show_trace` 의 Undiagnosed share 노드
...
`show_trace` 의 Rates 노드는 두 자리 (mortality_annual / DEATH(an)) 의 값을
