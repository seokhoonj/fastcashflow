...
* - 사망 (DEATH)
  - 사망
  - 계약 전체 `inforce` (별도 사망탈퇴율이 깎음)
  - 한 번
* - 진단 (DIAGNOSIS)
  - 진단
  - 그 담보의 `undiagnosed` 풀 (진단 발생률이 깎음)
  - 한 번
* - 입원 (MORBIDITY)
  - 입원
  - 없음 (아무 풀도 안 깎음)
...
**별도의 사망탈퇴율이 계약 풀을 깎는 데서** 나옵니다. 그래서 사망과 입원이
같은 (A)인데도 결과가 정반대입니다.
...
청구하면서 동시에 그 풀을 `(1 - 진단발생률)` 로 깎습니다. 사망에서 *별도의*
사망탈퇴율이 계약 풀을 깎던 자리를, 진단에서는 *같은* 진단 발생률이 자기
미진단 풀을 깎는 것 — 탈퇴와 발생이 한 입력에 묶입니다.
...
사망은 계약 전체 풀(별도 사망탈퇴율이 깎음), 진단은 자기 미진단 풀(진단
발생률이 깎음), 입원은 아무 풀도 아님. 청구 식은 셋 다 같고, 이 한 가지가
시계열의 모양을 완전히 바꿉니다.
