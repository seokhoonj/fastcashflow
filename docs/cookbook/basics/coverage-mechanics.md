...
```{mermaid}
flowchart TB
    DEATH["DEATH: 사망률이 inforce 를 감쇠<br/>사건 = inforce 감소 (한 번)"]
    MORB["MORBIDITY: inforce 안 줄임<br/>같은 계약 반복 청구 (여러 번)"]
    DIAG["DIAGNOSIS: 보장마다 자기 undiagnosed 풀<br/>풀이 (1-rate) 로 소진 (한 번)"]
```
