...
- 산출방법에 따라 어떤 계산 분기로 들어가는지 — 다섯 가지로 단순화
...
이런 식으로 지급된다"** 를 미리 알려줘야 합니다. 그 매핑이 담보별 산출방법
...
5. 생활비 / 재진단 / DI (장해소득) 같은 상태에 머문 기간에 따라 달라지는 산출방법은 v1 미지원 — Semi-Markov
   영역. (별도 phase)

담보별 산출방법은 **신담보가 추가될 때만** 한 줄을 더해줍니다. 분기 결산
때 갱신되는 건 `basis.xlsx` 입니다 — 담보별 산출방법이 분리되어 있어
...
* - V3
  ...
  - `coverages` 프레임의 담보 코드가 담보별 산출방법에 누락
* - V4
  - `measure()` 진입
...
`gmm.trace` 의 Coverages 섹션이 한 계약의 담보별 산출방법 매핑을 그대로
보여줍니다:
...
엔진이 산출방법에서 derive 한 값입니다.

## 인접 레시피

- [담보별 산출방법](coverage-mechanics) — 각 CalculationMethod 이
  엔진 안에서 어떤 알고리즘으로 처리되는지 (이 챕터는 담보별 산출방법 선택,
  메커니즘 챕터는 실행 알고리즘).
- [검증 패턴 — gmm.trace](../workflow/validation) — 담보별 산출방법 변경이
