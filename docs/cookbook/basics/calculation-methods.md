...
- 한국 상품의 매핑 표, 담보 산출방식 작성, 등록 누락 시 잡히는 자리
```
...
이런 식으로 지급된다"** 를 미리 알려줘야 합니다. 그 매핑이 *담보 산출방식*
...
자체를 모릅니다 — 담보 산출방식에 등록된 담보들 중 어느 것이 주계약인지는
회사 / product 단위 결정.
```
...
| 담보 산출방식 | `calculation_methods.csv` | 어떤 담보가 어떤 산출방식인가 |

세 파일이 각자 자기 책임만 가지면 한 갱신이 다른 파일에 닿지 않습니다.
담보 산출방식은 *신담보 추가* 작업의 자리, basis 는 *위험률 calibration*
...
mp = fcf.read_model_points(
    ...
    calculation_methods="calculation_methods.csv",  # ← 담보 산출방식
)
```
...
amount 지급). 차이는 *rate_table* 일 뿐. 담보 산출방식에 각자 별도
`coverage_code` 로 등록하되 산출방식은 모두 `DEATH`.
```

## 담보 산출방식 작성 — `calculation_methods.csv`
...
지급 rate 는 담보 산출방식에 등록된 DEATH 담보의 자체 `rate_table` 이
결정합니다.
```
...
담보 산출방식은 **신담보가 추가될 때만** 한 줄을 더해줍니다. 분기 결산
때 갱신되는 건 `assumptions.xlsx` 입니다 — 담보 산출방식이 분리되어 있어
결산 워크플로가 이 파일을 건드리지 않습니다.

## 함정 / 검증 — 등록 누락 시 어디서 에러

네 단계 검증이 담보 산출방식 미스를 catch 합니다:
...
* - V3
  - `_long_model_points`
  - `assumptions.xlsx` 의 rate-driven 담보가 담보 산출방식에 누락
* - V4
  - `measure()` / `value()` 진입
  - 담보 산출방식에 있는 rate-driven 코드의 rate_table 이 basis 에 없음
```
...
`show_trace` 의 Coverages 섹션이 한 계약의 담보 산출방식 매핑을 그대로
보여줍니다:
...
`method` 칸이 담보 산출방식에서 가져온 값, `risk` / `is_diagnosis` 는
엔진이 산출방식에서 derive 한 값입니다.

## 인접 레시피

- [보장 청구 메커니즘](coverage-mechanics) — 각 CalculationMethod 이
  엔진 안에서 어떤 알고리즘으로 처리되는지 (이 챕터는 *담보 산출방식 선택*,
  메커니즘 챕터는 *실행 알고리즘*).
- [검증 패턴 — show_trace](../workflow/validation) — 담보 산출방식 변경이
