...
- 출재사 관점: lapse SCR 을 줄여 지급여력비율을 끌어올리는 법 (`fcf.mass_lapse_reinsurance.cedant_solvency_relief`)
...
> (`fcf.mass_lapse_reinsurance`) 은 이미 구현 · 테스트되어 있으므로, 남은 일은 서술과
> 실행 예제입니다.
...
- `fcf.mass_lapse_reinsurance` — `LapseXL`, `cedant_solvency_relief`, `price_treaty`,
